...
const ClientState_js_1 = require("/static/ClientState.js");
...
let client_state = new ClientState_js_1.ClientState(SCREEN_DIMS, MAP_DIMS, MAP_VIEW_RATIO, true);
let MINIMAP_DIMS = { width: minimap.clientWidth, height: minimap.clientHeight };
let minimap_state = new ClientState_js_1.ClientState(MINIMAP_DIMS, MAP_DIMS, { width: 1, height: 1 }, false);
let player_count = 0;
var host = window.document.location.host.replace(/:.*/, '');
var client = new Colyseus.Client(location.protocol.replace("http", "ws") + "//" + host + (location.port ? ':' + location.port : ''));
// var client = new Colyseus.Client("wss://xq-zci.colyseus.dev");
/******* Button press registering variables *******/
let keys = new Set(), tankMoveInterval = null, allowedKeys = {
    'KeyW': true,
    'KeyS': true,
    'KeyA': true,
    'KeyD': true,
