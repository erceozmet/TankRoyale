...
class ClientState {
    ...
    add_gameobj(gameobj, index) {
        if (!this.show_loots && gameobj.height <= this.min_size.height && gameobj.width <= this.min_size.width)
            return;
        ...
        if (gameobj.height > this.max_object_size.height)
            this.max_object_size.height = gameobj.height;
        if (gameobj.width > this.max_object_size.width)
            this.max_object_size.width = gameobj.width;
    ...
    render_view() {
        ...
        for (; row < this.view_dims.height; row++) {
            ...
            for (; col < this.view_dims.width; col++) {
                ...
                let sprite = (_a = this.objects.at(row_index)) === null || _a === void 0 ? void 0 : _a.at(col_index);
                if (!sprite)
                    continue;
                sprite.visible = true;
                sprite.y = this.tile_size.height * row;
    ...
    wrap_view_pos() {
        ...
        if (this.view_pos.row < 0)
            this.view_pos.row = 0;
        if (this.view_pos.col < 0)
            this.view_pos.col = 0;
        if (this.view_pos.col + this.view_dims.width > this.map_dims.width)
            this.view_pos.col = this.map_dims.width - this.view_dims.width;
        if (this.view_pos.row + this.view_dims.height > this.map_dims.height)
            this.view_pos.row = this.map_dims.height - this.view_dims.height;
    }
}
